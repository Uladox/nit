...
test_map(const MunitParameter params[], void *data)
{
	...
	for (; i < MAP_MAX; ++i) {
		...
		*storage = i;
		munit_assert_false(map_add(map, &i, sizeof(i),
					   storage, &stack));
	}

	for (i = 0; i < MAP_MAX; ++i)
		munit_assert_int(i, ==,
				 *(int *) map_get(map, &i, sizeof(i)));
	...
	do {
		...
		int *val = map_iter_val(&iter);

		munit_assert_int(*key, ==, *val);
	} while (++i, map_iter_next(&iter));

	munit_assert_int(i, ==, MAP_MAX);
	i = 42;
	free(map_remove(map, &i, sizeof(i), &stack));
	munit_assert_null(map_get(map, &i, sizeof(i)));
	map_free(map, map_free_contents, NULL);
	free(stack);
	return MUNIT_OK;
}
